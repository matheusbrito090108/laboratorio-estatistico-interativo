...
def render_lgn():
    st.header("1. Lei dos Grandes Números (LGN)")
    ...
    st.info(f"**Resultado final:** Média obtida = **{med_acumulada[-1]:.4f}** (Valor esperado = **{p}**)")

def render_tcl():
    st.header("2. Teorema Central do Limite (TCL)")
    ...
    st.success(f"A distribuição das médias de uma população **{dist_tipo}** aproxima-se de uma **Distribuição Normal** à medida que o tamanho da amostra ($n$) aumenta.")
